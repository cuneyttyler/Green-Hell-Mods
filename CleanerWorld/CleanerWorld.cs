...
namespace CleanerWorld
{
    ...
    public class AcreExtended : Acre, IItemSlotParent
    {
        public new void OnInsertItem(ItemSlot slot, bool from_replication)
        {
            try
            {
                if (slot == null || slot.m_Item == null || from_replication)
                {
                    return;
                }
                ...
                else
                {
                    ...
                    if (EnumTools.IsItemSpoiled(slot.m_Item.m_Info.m_ID))
                    {
                        m_FertilizerAmount += slot.m_Item.m_Info.m_FertilizeAmount;
                        m_FertilizerAmount = Mathf.Clamp(m_FertilizerAmount, 0f, m_MaxFertilizerAmount);
                    ...
                    float waterAmount = m_WaterAmount;
                    m_WaterAmount += liquidContainerInfo.m_Amount;
                    m_WaterAmount = Mathf.Min(m_WaterAmount, m_MaxWaterAmount);
                    float num = m_WaterAmount - waterAmount;
                    liquidContainerInfo.m_Amount -= num;
                    slot.RemoveItem();
                    InventoryBackpack.Get().InsertItem(item);
                    if (num > 0f)
                    {
                        PlayerAudioModule.Get().PlayWaterSpillSound();
                    }
                }
            }
            catch (Exception ex)
    ...
    public class BalanceSystem20Extended : BalanceSystem20
    {
        public override void OnItemDestroyed(CJObject balance_object)
        {
            try
            {
                bool main_obj_destroyed = false;
                Item component = balance_object.GetComponent<Item>();
                if (component && component.m_DestroyedFromBalanceSystemRepl)
                {
                    return;
                }
                ...
                {
                    Item topMostItem = this.GetTopMostItem(balance_object.gameObject);
                    if (topMostItem != null && topMostItem.m_Info != null && topMostItem.m_Info.m_Health <= 0f)
                    {
                    ...
                    {
                        if (this.ItemDestroyed(group, gameObject, main_obj_destroyed, balance_object.gameObject))
                        {
                            flag = true;
                            break;
                        }
                    ...
                    if (!flag && gameObject.transform.parent != null)
                    {
                        ...
                        {
                            if (this.ItemDestroyed(group2, gameObject, false, balance_object.gameObject))
                            {
                                break;
                            }
                        }
